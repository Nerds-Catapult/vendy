...
const Header = () => {
  ...
  const nextSlide = () => {
    setCurrentIndex((prevIndex) =>
      Math.min(prevIndex + 1, embeddedImagesArray.length - visibleImages)
    );
  };
  ...
  return (
    <div className="">
      <main className="p-2 w-full">
        <section className=" py-4 w-full pl-5 pr-5 rounded-md sm:h-200px bg-red-400 ">
          <div className="flex justify-between">
            <div className="flex w-full">
              ...
              <div className="flex gap-2 sm:pl-6 lg:pl-[82px] justify-start w-full overflow-hidden p-4 lg:gap-[120px]">
                {embeddedImagesArray
                  .slice(currentIndex, currentIndex + visibleImages)
                  .map((image, index) => (
                    <div key={index} className="min-w-32 h-32">
        ...
        {/*best sellers*/}
        <h1 className="font-bold text-2xl pl-4 py-4">Our Best Sellers</h1>
        <section className="grid grid-cols-2 lg:grid-cols-3 gap-4 px-2">
        ...
        <section className="bg-gray-100 mt-4 pb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {brandings.map((image, index) => (
              <div
                key={index}
                className="text-center object-cover flex justify-center w-full"
              >
                <Image
                  src={image}
                  alt="logo"
